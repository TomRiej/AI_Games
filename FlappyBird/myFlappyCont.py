...
class Flappy:
    def __init__(self, master, popSize, gravity, refreshDelay, jumpStrength, pipeGap, pipeVelocity, spawnRate):
        ...
        self.canvas.bind("s", self.onSave)
...
# ========================== Game Logic =========================================
    ...
    def findClosestPipeIndex(self):
        if (self.pipes[0].x + self.pipes[0].width) > (self.allBirds[0].x - self.allBirds[0].radius):
            return 0
        else:
            return 1
...
# ============================= Game stuff ===========================================
    ...
    def newgame(self, initBirds):
        ...
        self.graphText = self.canvas.create_text(130,780, text="press space to see graph", font="Verdana 20")
        self.scoreText = self.canvas.create_text(60,20, text=("best: "+ str(self.bestScore)), font="Verdana 20")
        self.genText = self.canvas.create_text(50,40, text=("Gen: "+ str(self.generationCounter)), font="Verdana 20")
        ...
        self.cScoreText = self.canvas.create_text(400, 50, text=str(self.activeBirds[0].score), font="Verdama 30")
    ...
    def refresh(self):
        ...
        # moving birds
        pipeInputs = closestPipe.getPipeInputs()   
        for i in range(len(self.activeBirds)):
            ...
            self.activeBirds[i].y += self.activeBirds[i].velocity 
        ...
        for pipe in range(len(self.pipes)):
            ...
            self.pipes[pipe].x += self.pipeVelocity 
        ...
        # update score label
        self.canvas.itemconfig(self.cScoreText, text=str(self.activeBirds[0].score))
        ...
        if len(self.activeBirds) == 0:
            ...
            self.yData.append(self.bestScore)
            self.xData.append(self.generationCounter)
